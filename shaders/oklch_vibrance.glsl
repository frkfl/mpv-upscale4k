...
//!PARAM ov_chroma_tame
//!TYPE float
0.15

//!HOOK MAIN
//!BIND HOOKED
//!DESC [Custom] Cinema-style vibrance (HDR-safe)
...
vec4 hook() {
    ...
    float g_in = max(ov_gamma_in, 0.01);
    float g_out = max(ov_gamma_out, 0.01);
    ...
    // Lightness gate: favor midtones
    float l_lo = smoothstep(0.08, 0.30, L);
    float l_hi = 1.0 - smoothstep(0.75, 0.95, L);
    float l_mid = l_lo * l_hi;

    // Chroma gate: favor low/mid chroma, avoid nuking already-saturated stuff
    ...
    float shape_gate = mix(1.0, vib_shape, clamp(ov_mid_bias, 0.0, 1.0));

    // Skin protection (hue from encoded space)
    float h_src = safe_hue(src.rgb);
    float dh = min(abs(h_src - ov_skin_hue), 1.0 - abs(h_src - ov_skin_hue));
    float skin_gate = 1.0 - ov_skin_protect * exp(-0.5 * pow(dh / max(ov_skin_width, EPS), 2.0));

    // Vibrance: boost low-sat colors more than high-sat ones
    ...
    float max_boost = 1.5 * ov_vibrance + 1.0;
    ...
    float tame = max(ov_chroma_tame, 0.0);
    if (tame > 0.0) {
