...
//!PARAM msh_flat_protect
//!TYPE float
0.7

//!PARAM msh_motion_gate
//!TYPE float
0.5
...
vec4 hook() {
    ...
    // Clamp correction
    float corr = clamp(hi, -msh_clamp_amt, msh_clamp_amt);

    // Edge band mask
    float band = smoothstep(msh_edge_low, msh_edge_high, edge) *
                 (1.0 - smoothstep(msh_edge_high, msh_edge_high * 2.0, edge));

    // Flat area protection
    float var = abs(y - ((yx1 + yx0 + yy1 + yy0) * 0.25));
    float flat = 1.0 - smoothstep(0.0, 0.08, var);
    float protect = mix(1.0, flat, msh_flat_protect);

    // Motion gating (previous frame luma)
    float yprev = luma(PREV_tex(uv).rgb);
    float motion = clamp(abs(y - yprev) * 4.0, 0.0, 1.0);
    float gate = mix(1.0, 1.0 - motion, msh_motion_gate);

    // Total sharpen gain
    float gain = msh_strength * band * protect * gate;
