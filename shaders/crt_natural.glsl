...
//!PARAM cn_bloom_strength
//!TYPE float
0.06

//!PARAM cn_bloom_radius
//!TYPE float
1.25

//!PARAM cn_mask_strength
//!TYPE float
0.18
...
vec4 hook() {
    ...
    vec3 lin = toLinear(rgb, cn_gamma_in);
    ...
    float scanW = 1.0 - cn_scan_amp * pow(wave * wave, max(cn_scan_curve, 1.0));
    vec3 sl_col = lin * scanW;

    // 3) Local bloom (5-tap, linear domain)
    float r = cn_bloom_radius;
    vec3 s1 = toLinear(HOOKED_tex(uv + vec2( px.x * r, 0.0)).rgb, cn_gamma_in);
    vec3 s2 = toLinear(HOOKED_tex(uv + vec2(-px.x * r, 0.0)).rgb, cn_gamma_in);
    vec3 s3 = toLinear(HOOKED_tex(uv + vec2(0.0,  px.y * r)).rgb, cn_gamma_in);
    vec3 s4 = toLinear(HOOKED_tex(uv + vec2(0.0, -px.y * r)).rgb, cn_gamma_in);
    vec3 avg = (sl_col * 2.0 + s1 + s2 + s3 + s4) / 6.0;
    vec3 withBloom = mix(sl_col, avg, clamp(cn_bloom_strength, 0.0, 1.0));
    ...
    float soft = mix(1.0, smoothMaskStep(fx, clamp(cn_mask_soft, 0.0, 1.0)), clamp(cn_mask_soft, 0.0, 1.0));
    vec3 mask = mix(vec3(1.0), m, clamp(cn_mask_strength, 0.0, 1.0) * soft);
    vec3 masked = withBloom * mask;

    // 5) Luma compensation to preserve colorfulness
    float Y1 = max(1e-6, luma(masked));
    float gain = mix(1.0, Y0 / Y1, clamp(cn_chroma_preserve, 0.0, 1.0));
    vec3 comp = masked * gain;

    // 6) Gentle saturation boost (luma-preserving)
    float Yc = luma(comp);
    vec3 satCol = mix(vec3(Yc), comp, cn_sat_boost);

    // 7) Slight black lift to avoid crush
    satCol = max(satCol, vec3(cn_black_lift));

    // 8) Back to display gamma
    vec3 outc = toGamma(satCol, cn_gamma_out);
